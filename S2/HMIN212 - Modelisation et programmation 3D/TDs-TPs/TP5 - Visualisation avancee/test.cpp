...
void draw_cube(const Point3& pos) {
	
	glBegin(GL_QUADS);

	// TOP FACE
	Vector3 normal5 = normal(Point3(-1, 1, -1), Point3(-1, 1, 1), Point3(1, 1, 1)).normalized();
	...
	Vector3 normal3 = normal(Point3(1, 1, 1), Point3(1, -1, 1), Point3(1, -1, -1)).normalized();
...
void renderScene() {
	...
	glBegin(GL_POINTS);
		glColor3d(0,1,0);glVertex4fv(light0Pos);
