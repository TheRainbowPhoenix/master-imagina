...
Point3 Point3::project_on_line(Point3 origin, Vector3 direction){
	...
	Vector3 me_to_line = start_to_me.dot(direction) / direction.length();
	return origin + direction.normalized() * me_to_line.length(); 
}
...
Point3 operator+(const Point3& p, const Vector3& v){
	return Point3(p.x + v.x, p.y + v.y, p.y + v.y);
}
