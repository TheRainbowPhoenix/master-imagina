...
to-report vect-align
  let x-component sum [dx] of visible-neighbors
  let y-component sum [dy] of visible-neighbors
...
to-report vect-cohere
  let x-component mean [sin (towards myself + 180)] of visible-neighbors
  let y-component mean [cos (towards myself + 180)] of visible-neighbors
  report (list x-component y-component)
end
...
to vect-direction
  let Vd vect-from-angle heading speed
  set Vd adjust
  set happy? true

  let a angle-from-vect Vd
  let r length-vect Vd
...
to-report adjust ;; turtle procedure
