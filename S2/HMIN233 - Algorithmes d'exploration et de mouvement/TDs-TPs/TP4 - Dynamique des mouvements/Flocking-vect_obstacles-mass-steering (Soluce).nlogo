...
to setup
  ...
  crt population
    [ ;;set color green - 2 + random 7  ;; random shades look nice
      ...
      set acceleration 0
      set mass random-float mass-range
...
to go
  ask turtles [ flock ]
  ask turtles [update-speed fd speed ]
  tick
end
...
to flock  ;; turtle procedure
  ...
  [let  v  vectDirect
    ...
    let f 0
    if friction? [set f friction]
    set acceleration (s * amount-acceleration) / mass - f
  ]
  [ if any? obstacles
    ...
    let f 0
    if friction? [set f friction]
    set acceleration (base-speed / 20 * amount-acceleration) / mass - f
 ;   if acceleration < 0 [print (sentence "acceleration negative" acceleration ) ]
  ]
end
...
;    turn-towards a max-angle-turn ]
;    ;;[ let a angleFromVect (additionvect (list heading 1) vectObstacles)
;    ;;turn-towards a max-angle-turn ]
;
;;  if any? flockmates
...
to find-obstacles
  set obstacles patches in-cone 9 60 with [pcolor = red]
end
