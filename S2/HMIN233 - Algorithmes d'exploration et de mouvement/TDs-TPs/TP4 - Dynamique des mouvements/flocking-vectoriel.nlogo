...
to-report next-speed
  let friction 0
  if friction? [
    set friction factor-friction * speed
  ]
  report (speed * acceleration) - friction
end
...
to-report vect-direction
  let Va multiply-vect factor-align vect-align
  let Vs multiply-vect factor-separate vect-separate
  let Vc multiply-vect factor-cohere vect-cohere

  let Vd vect-add Va Vs
  set Vd vect-add Vd Vc
  report Vd
end
