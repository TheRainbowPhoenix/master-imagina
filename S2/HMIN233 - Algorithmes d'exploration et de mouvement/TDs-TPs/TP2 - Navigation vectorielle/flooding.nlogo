...
to compute-potential1
  ask patches [
    if goal != 1 and obstacle != 1[
      ...
      let old-pot potential
      let case1 max-one-of neighbors [potential]
      let p ([potential] of case1) - 1

      if potential <= p and p != old-pot [

        set potential p

        ifelse potential < 0 [
          set potential 0
          set pcolor scale-color orange potential 0 (potential-level * 1.2)
        ][
          set continue false
...
to compute-repulsive1
  ask patches [
    if (goal = 0) and (obstacle = 0) [
      ...
      let pot min-one-of neighbors [repulsive]
      let p [potential] of pot + repulsive-increment
