__includes ["../libnlogo/vector2.nls"]
...
to draw-obstacles
  let tare (obstacle-size - 1) / 2
  let i obstacle-size
  ;ask patch 0 0 [set pcolor red]
  while [i > 0] [
    let j obstacle-size
    while [j > 0] [
      ask patch (i - tare) (j - tare) [set pcolor red]
      set j j - 1
    ]
    set i i - 1
  ]
...
to manage-heading
  let V-flock vect-flock
  let V-avoid vect-multiply vect-avoid factor-avoid

  let factor length-vect V-avoid
  let V-direction vect-add (vect-multiply V-avoid factor) (vect-multiply V-flock (1 - factor))

  let angle angle-from-vect V-direction

  turn-towards angle max-agent-turn
end
...
to-report vect-flock  ;; turtle procedure
  find-flockmates
  ifelse any? flockmates [
    let V-align vect-multiply vect-align factor-align
    let V-separate vect-multiply vect-separate factor-separate
    let V-cohere vect-multiply vect-cohere factor-cohere

    let V-flocking vect-add V-align V-separate
    set V-flocking vect-add V-cohere V-flocking

    report V-flocking
  ][
    report vect-from-angle heading 0.0000000001
  ]
end

to-report vect-avoid
  set obstacles patches in-cone vision vision-cone-angle with [pcolor != black]
  ifelse any? obstacles [
    let proche min-one-of obstacles [distance myself]
    report vect-from-angle (towards proche + 180) ((distance proche) ^ 2)
  ][
    report vect-from-angle heading 0.0000000001
  ]
end
...
to-report vect-separate
  ...
  ifelse nearest-neighbor = nobody [
    ...
    report vect-from-angle (towards nearest-neighbor + 180) (1 / distance nearest-neighbor)
  ]
end
