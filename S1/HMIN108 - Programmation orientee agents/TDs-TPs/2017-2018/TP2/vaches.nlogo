patches-own[cpt-temps cpt-temps-init taille-plante croit]

breed [taureaux taureau]
breed [vaches vache]
breed [lions lion ]

vaches-own[energie gain-brouter sick cpt]
lions-own[energie gain-devore cpt sick]
taureaux-own[energie gain-brouter]

to setup
  clear-all
  set-default-shape vaches "cow"
  set-default-shape taureaux "cow skull"
  set-default-shape lions "wolf" 
  create-vaches nbr-vaches [
    set color white
    ...
    set gain-brouter 5
    set sick random 55]
  ask vaches[color-sick]
  create-taureaux nbr-taureaux [
    set color red
    setxy random-xcor random-ycor
    set size 2]
  create-lions nbr-lions [
    set color yellow 
    setxy random-xcor random-ycor
    set energie 0
    set gain-devore 20
  ...
  ask patches[
    set cpt-temps-init random temps-croissance-max
...
to time-up-taille
 ...
 ; [ ifelse croit = 1
    [ set taille-plante taille-plante + 1
     ;  ]
   ; [ set taille-plante taille-plante - 1 ]
 ; if taille-plante >= taille-plante-max OR taille-plante <= 0
  ;[ ifelse croit = 1
   ; [ set croit 0 ]
   ; [ set croit 1 ]]
    set pcolor scale-color green taille-plante 0 500
    set cpt-temps cpt-temps-init ]
end

to update-plot
  set-current-plot "herbe"
  set-current-plot-pen "herbe"
  plot sum [taille-plante] of patches
end

to broute
  ifelse taille-plante > consom-vaches
  [ set taille-plante taille-plante - consom-vaches 
...
to follow-chief
  let chief one-of taureaux in-radius 7
  ifelse chief != nobody
  [ set heading towards chief
    wiggle ]
  [ wiggle ]
end

to bouffe
  let prey one-of vaches in-radius percep
  if prey != nobody
  [  if [sick] of prey = 1
    [  die 
      ask prey [ die ]
    ...
    set energie energie + gain-devore
  ]
  wiggle 
end
