...
; Copyright 1997 Uri Wilensky, Jacques Ferber (for the FSM version).
...
## FSM Machine (extension from the Termites behavior)

This model makes use of a FSM (i.e. Finite Stage Machine) which simplifies the coding of the termites.
The FSM itself is simply a run ctask behavior, where cstask is an attribute of the turtles, which contains the procedure to run.
At each tick, the turtle run the procedure whose name is set in the ctask variable.
...
(Rewriting for FSM architecture, Copyright 2014, Jacques Ferber)
