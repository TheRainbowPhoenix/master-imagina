...
to go  ;; turtle procedure
  let chip-color search-for-chip
  find-new-pile chip-color
 ; find-pile 
 ; find-color-pile
  put-down-chip chip-color
end

to-report search-for-chip  ;; turtle procedure -- "picks up chip" by turning orange

  ifelse pcolor != black
  [ let chip-color pcolor
    set pcolor black
    set color orange
    fd 20
    report chip-color ]
  [ wiggle
    report search-for-chip ]
end

to find-new-pile [chip-color]  ;; turtle procedure -- look for yellow patches
  ifelse chip-color = yellow
  [if pcolor = black
  [ wiggle
    find-new-pile chip-color ]
...
to put-down-chip [chip-color] ;; turtle procedure -- finds empty spot & drops chip
