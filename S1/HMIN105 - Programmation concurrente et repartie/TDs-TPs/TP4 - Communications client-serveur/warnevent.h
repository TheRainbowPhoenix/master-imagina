...
fprintf(LOG_OUTPUT, "[%s] (%s:%d) in \'%s\' : " FMT, LOG_TYPE, __FILE__, __LINE__, __func__, ##__VA_ARGS__)
...
// Verifie si une erreur s'est produit sur un stream et applique
// une action adapté.
#define WARN_ERROR_STREAM(STREAM) \
WARN_EVENT(ferror(STREAM), \
	WARN_DEF_OUTPUT, \
	WARN_DEF_TYPE, \
	"stream function returned error code", \
	WARN_DEF_ACTION, \
	"\n[ERRNO] (%s)\n", \
	strerror(errno))
...
WARN_EVENT(RES != 0, \
	WARN_DEF_OUTPUT, \
	WARN_DEF_TYPE, \
	"pthread function returned error code", \
	WARN_DEF_ACTION, \
	"\n[ERRNO] (%s)\n", \
	strerror(errno))
...
WARN_EVENT(RES != 0, \
	WARN_DEF_OUTPUT, \
	WARN_DEF_TYPE, \
	"getaddrinfo function returned error code", \
	WARN_DEF_ACTION, \
	"\n[ERRNO] (%s)\n", \
	gai_strerror(errno))
