...
public class ConstraintExt extends Constraint{
	...
	public boolean violation(Assignment a) {
		...
		for(ArrayList<Object> tuple : this.tuples){ //cycle through all the constraint's tuples, which are a list of values in order
			...
			for(String variable : a.getVars()){ //cycles through all the assignment's variables
				if(this.getVars().contains(variable)){ //if the constraint includes this variable then check that the assignment for this variable corresponds to the value for this variable in the current tuple
					if(!((a.get(variable)).equals(tuple.get(this.getVars().indexOf(variable))))){ //if it doesn't, then move on to the next tuple and start again
						validTuple = false;
					}
				}
				if(!validTuple) break;
